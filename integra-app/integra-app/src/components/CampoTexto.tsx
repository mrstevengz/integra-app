...
export function CampoTexto<T extends FieldValues>({
  ...
  const [codigoPais, numero] = esTelefono
    ? separarTelefono(field.value as string)
    : ["", ""];
  ...
  return (
    <View className="mb-4">
      <Text className="mb-2 text-label font-lexend-bold font-medium text-content">
        {title}
        {opcional && (
          <Text className="text-label font-lexend text-content-muted">
            {" "}
            (opcional)
          </Text>
        )}
      ...
      {esTelefono && (
        <View className="flex-row gap-2">
          <View
            className={`w-24 flex-row items-center rounded-chip border bg-surface-raised px-3 ${borde("codigoPais")}`}
          >
            <Text className="font-lexend text-[15px] text-content-muted">
              +
            </Text>
            ...
            <TextInput
              value={codigoPais}
              onChangeText={(texto) =>
                field.onChange(unirTelefono(texto.replace(/\D/g, ""), numero))
              }
          ...
          <TextInput
            ...
            value={numero}
            onChangeText={(texto) =>
              field.onChange(unirTelefono(codigoPais, texto))
            }
      ...
      {esContrasenia && (
        <View
          className={`flex flex-row items-center rounded-chip border bg-surface-raised px-3 ${borde("principal")}`}
        >
          <TextInput
            placeholder={placeholder}
            ...
            textAlignVertical="center"
            placeholderTextColor={color.contentDisabled}
            className="flex-1 font-lexend py-3 text-[15px] text-content"
          />

          {secureTextEntry ? (
            <Eye size={25} onPress={presionarIcono} className="ml-2" />
          ) : (
            <EyeOff size={25} onPress={presionarIcono} className="ml-2" />
          )}
        </View>
      )}

      {!esContrasenia && !esTelefono && (
        <TextInput
          placeholder={placeholder}
          value={(field.value as string) ?? ""}
          onChangeText={field.onChange}
